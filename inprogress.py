...
def display_timer(screen):
    ...
    timer_surface = test_font2.render(f'{current_time}',False,(64,64,64)) #curren_time NEED TO BE AN INT
...
def create_obstacle(current_time, screen, obstacle_group):
        if current_time % 10 == 0 and len(obstacle_group) < 15  and  current_time != 0 :
            type = randint(0,3)
            if type <2:
...
pygame.display.set_caption('Endless Runner') #setting the name of the game
clock = pygame.time.Clock()
test_font = pygame.font.Font('freesansbold.ttf',30) #font type and font size
test_font2 = pygame.font.Font('8-BIT WONDER.ttf',15)
game_active = False
start_time = 0
# music
...
#timer
timer_surface = test_font2.render('Timer', False, 'Black') #text you want to display, anti anliasing(False or True) , color
timer_rect = timer_surface.get_rect(topleft = (600,47 ))


#intro texts
intro_text = test_font2.render('Press Enter to start the game', False, 'Black')
...
#inputs
    ...
    #score        
        points.show_score()
        for obstacle in obstacle_group:
            if obstacle.rect.x <= 555 and obstacle.rect.x >= 545:
            #if p1.rect.x == (obstacle.rect.x):
                points.increment_score()

    #Dficulty
        if points.actual_score % 20 == 0 and points.actual_score != 0:
            test += 1
            
        if test == 1:
            plus += 1
        
        if not points.actual_score % 20 == 0:
            test = 0
    ...
    if game_active == False:
        ...
        points.show_high_score()
        points.actual_score = 0
